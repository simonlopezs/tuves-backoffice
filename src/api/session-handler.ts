...
export class SessionHandler {
  ...
  private async getUserData(uid: string) {
    return this.dbConnector.getUser(uid).then((user) => {
      if (user) {
  ...
  async logout() {
    return signOut(this.auth).then(() => this.dbConnector.setTenant(null));
  }

  subscribeToSessionState(fn: (user: IUser | null) => void) {
